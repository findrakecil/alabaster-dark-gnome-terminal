...
# Writes the Alabaster Dark GNOME Terminal theme colors and configurations as dconf key-value pairs to the target profile.
#
...
apply() {
  local \
    primary_white="#CECECE" \
    primary_black="#0E1415" \
    normal_black="#0E1415" \
    normal_red="#E25D56" \
    normal_green="#73CA50" \
    normal_yellow="#E9BF57" \
    normal_blue="#4A88E4" \
    normal_magenta="#915CAF" \
    normal_cyan="#23ACDD" \
    normal_white="#F0F0F0" \
    bright_black="#777777" \
    bright_red="#F36868" \
    bright_green="#88DB3F" \
    bright_yellow="#F0BF7A" \
    bright_blue="#6F8FDB" \
    bright_magenta="#E987E9" \
    bright_cyan="#4AC9E2" \
    bright_white="#FFFFFF"

  _write palette "['$normal_black', '$normal_red', '$normal_green', '$normal_yellow', '$normal_blue', '$normal_magenta', '$normal_cyan', '$normal_white', '$bright_black', '$bright_red', '$bright_green', '$bright_yellow', '$bright_blue', '$bright_magenta', '$bright_cyan', '$bright_white']"
  log 4 "Applied Alabaster Dark color palette"

  _write background-color "'$primary_black'"
  _write foreground-color "'$primary_white'"
  _write use-transparent-background "false"
  log 4 "Applied background- and foreground colors"

  _write bold-color "'$primary_white'"
  ...
  _write cursor-foreground-color "'$primary_black'"
  _write cursor-background-color "'$primary_white'"
  ...
  log 4 "Set Alabaster Dark GNOME Terminal version key of the '$profile_name' profile"

  log 3 "Applied theme colors and configurations"
}

# Applies the Alabaster Dark GNOME Terminal theme with version comparison.
#
...
apply_version_compared() {
  ...
  version=$1
  case "$(
    vercomp "$ALABASTER_DARK_GNOME_TERMINAL_VERSION" "$version"
    echo $?
  )" in
    0)
      log 3 "Reinstalling Alabaster Dark GNOME Terminal since the version equaled the version of the '$profile_name' profile!"
      apply
      log 2 "Reinstalled Alabaster Dark GNOME Terminal version $ALABASTER_DARK_GNOME_TERMINAL_VERSION for the '$profile_name' profile!"
      exit 0
      ;;
    1)
      log 4 "The script version is newer than the currently installed theme ($ALABASTER_DARK_GNOME_TERMINAL_VERSION > $version)"
      apply
      log 2 "The '$profile_name' profile has been updated successfully from version $version to $ALABASTER_DARK_GNOME_TERMINAL_VERSION"
      exit 0
      ;;
    2)
      log 1 "The detected Alabaster Dark GNOME Terminal version $version of the '$profile_name' profile is greater than the version that will be applied!"
      printf "${_ctb}> [?]${_ct} Override current profile version %s with script version %s? (${_ctb}y${_cr}/${_ctb}n${_cr})" "$version" "$ALABASTER_DARK_GNOME_TERMINAL_VERSION"
      read -r -n 1 -s confirmation
      echo
      case $confirmation in
        [Yy]*)
          apply
          log 2 "Alabaster Dark GNOME Terminal version $ALABASTER_DARK_GNOME_TERMINAL_VERSION has been successfully applied!"
          exit 0
          ;;
        [Nn]*)
...
cleanup() {
  ...
  unset -v ALABASTER_DARK_GNOME_TERMINAL_SCRIPT_OPTS ALABASTER_DARK_GNOME_TERMINAL_VERSION ALABASTER_DARK_GNOME_TERMINAL_VERSION_DCONF_KEY ALABASTER_DARK_PROFILE_VISIBLE_NAME log_level DEPENDENCIES DCONF_PROFILE_BASE_PATH GSETTINGS_PROFILELIST_PATH gnome_terminal_version profile_name profile_uuid
...
clone_default_profile() {
  ...
  dconf write "$DCONF_PROFILE_BASE_PATH"/:"$profile_uuid"/visible-name "'$ALABASTER_DARK_PROFILE_VISIBLE_NAME'"
  append_profile_uuid_to_list "$profile_uuid"
  log 3 "Cloned the default profile '$uuid' with new UUID '$profile_uuid'"
}

# Gets the current Alabaster Dark GNOME Terminal version.
#
# @globread ALABASTER_DARK_GNOME_TERMINAL_VERSION_DCONF_KEY DCONF_PROFILE_BASE_PATH
...
get_current_version() {
  ...
  uuid=$1
  version=
  printf "$(dconf read $DCONF_PROFILE_BASE_PATH/:"$uuid"/$ALABASTER_DARK_GNOME_TERMINAL_VERSION_DCONF_KEY | tr -d "'")"
  return 0
}
...
ALABASTER_DARK_GNOME_TERMINAL_SCRIPT_OPTS=$(getopt -o hl:p: --long help,loglevel:,profile: -n 'install.sh' -- "$@")
ALABASTER_DARK_GNOME_TERMINAL_VERSION=1.0.0
ALABASTER_DARK_GNOME_TERMINAL_VERSION_DCONF_KEY=alabaster-dark-gnome-terminal-version
ALABASTER_DARK_PROFILE_VISIBLE_NAME="Alabaster Dark"
log_level=2
...
eval set -- "$ALABASTER_DARK_GNOME_TERMINAL_SCRIPT_OPTS"
while true; do
  case "$1" in
    --loglevel=*)
      log_level=${1#*=}
...
if validate_dependencies DEPENDENCIES[@]; then
  ...
  if [[ -n $profile_name ]]; then
    ...
    if [[ -n $profile_uuid ]]; then
      ...
      if [[ -n $curr_ver ]]; then
        ...
        apply
        log 2 "Alabaster Dark GNOME Terminal version $ALABASTER_DARK_GNOME_TERMINAL_VERSION has been successfully applied to the '$profile_name' profile"
        exit 0
      fi
    else
  ...
  profile_uuid="$(get_profile_uuid_by_name $ALABASTER_DARK_PROFILE_VISIBLE_NAME)"
  if [[ -n $profile_uuid ]]; then
    profile_name="$ALABASTER_DARK_PROFILE_VISIBLE_NAME"
    log 4 "Found already existing '$ALABASTER_DARK_PROFILE_VISIBLE_NAME' profile with UUID '$profile_uuid'"
    log "Updating already existing '$ALABASTER_DARK_PROFILE_VISIBLE_NAME' profile"
    apply_version_compared "$(get_current_version $profile_uuid)"
  fi

  clone_default_profile
  apply
  log 2 "Alabaster Dark GNOME Terminal version $ALABASTER_DARK_GNOME_TERMINAL_VERSION has been successfully applied to the newly created '$ALABASTER_DARK_PROFILE_VISIBLE_NAME' profile"
  exit 0
else
